...
def generate_avatar_with_reference(character_name, prompt_text, output_dir,
    ...
    # 上传参考图片
    ref_name = f"ref_{character_name}_{int(time.time())}.png"
    char_name = f"char_{character_name}_{int(time.time())}.png"
    ...
    if not upload_result1:
        print("上传参考图失败")
        return None
    print(f"参考图上传成功: {upload_result1.get('name', ref_name)}")
    ...
    if not upload_result2:
        print("上传人设图失败")
        return None
    print(f"人设图上传成功: {upload_result2.get('name', char_name)}")

    # 使用上传后的文件名
    ref_name = upload_result1.get('name', ref_name)
    char_name = upload_result2.get('name', char_name)
